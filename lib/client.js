...
function openFileTab(path) {
  ...
  if (!t) {
    t = { id: mkId("f"), kind: "file", path, ws: currentWs };
    tabs = tabs.concat(t);
  }
  activeTabId = t.id;
  persistTabs();
...
function setWorkspace(root) {
  ...
  activeTabId = s && s.activeTabId && tabs.some((t) => t.id === s.activeTabId) ? s.activeTabId : (tabs.find((t) => t.kind === "files") || tabs[0] || {}).id ?? null;
  if (assigned) {
  }
  persistTabs();
  notifyTabs();
...
function TabStrip() {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "fsviewer-tabstrip", style: { display: "flex", alignItems: "center", gap: 4, flex: "1 1 auto", minWidth: 0, overflowX: "auto", padding: "6px 0 6px 8px" } }, visibleTabs.map((t) => {
    const active = t.id === activeTabId;
    return /* @__PURE__ */ React.createElement(React.Fragment, { key: t.id }, t.kind === "browser" && firstBrowser && firstBrowser.id === t.id && visibleTabs[0].kind !== "browser" ? /* @__PURE__ */ React.createElement("span", { className: "fsviewer-tab-divider" }) : null, /* @__PURE__ */ React.createElement(
      "span",
      {
...
function FileTreePanel({ workspaces, sessions, sessionId }) {
  ...
  React.useEffect(() => {
    ...
    if (prev) snapshotWsState(prev, { panelOpen: panelOpen2, expanded: state.expanded, treeOn, treeW });
    const s = wsRoot ? getWsSession(wsRoot) : null;
    if (s && s.panelOpen != null) {
      if (s.panelOpen && !panelOpen2) openPanelWithRoom();
      else if (!s.panelOpen && panelOpen2) closePanel();
    }
  ...
  React.useEffect(() => {
    if (!wsRoot) return;
    snapshotWsState(wsRoot, { panelOpen: panelOpen2, expanded: state.expanded, treeOn, treeW });
  }, [wsRoot, panelOpen2, state.expanded, treeOn, treeW]);
  React.useEffect(() => {
    const col = document.querySelector('[class*="detailsCol"]');
