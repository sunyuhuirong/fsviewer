...
function setWorkspace(root) {
  ...
  activeTabId = s && s.activeTabId && tabs.some((t) => t.id === s.activeTabId) ? s.activeTabId : null;
  if (activeTabId == null) {
    let files = tabs.find((t) => t.kind === "files");
    if (!files) {
      files = { id: "files", kind: "files" };
      tabs = tabs.concat(files);
    }
...
function FileTreePanel({ workspaces, sessions, sessionId }) {
  ...
  React.useEffect(() => {
    const p = activeTab && activeTab.kind === "file" && (!activeTab.ws || activeTab.ws === currentWs) ? activeTab.path : null;
