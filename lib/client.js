...
function resolveCurrentWs(sessionId) {
  try {
    ...
    const hit = sid && items.find((w) => w.sessionIds.includes(sid)) || items.find((w) => w.workspaceId === snap.recentWorkspaceId) || (cwd ? items.find((w) => w.path === cwd) : null);
    if (hit) return { key: hit.workspaceId, path: hit.path };
    if (cwd) return { key: "cwd:" + cwd, path: cwd };
  } catch (e) {
    console.error("[fsviewer] resolve workspace:", e);
  }
  return null;
}
...
function setWorkspace(key, path) {
  ...
  if (p.panelOpen != null) {
    if (p.panelOpen) {
      openPanelWithRoom();
      setTimeout(() => {
        if (currentWs === key && p.panelOpen) openPanelWithRoom();
      }, 80);
...
let panelGuardWatch = null;
function stopPanelGuardWatch() {
  if (panelGuardWatch) {
    clearInterval(panelGuardWatch);
    panelGuardWatch = null;
  }
}
...
function startPanelResizeWatch() {
  ...
  panelResizeWatch = () => {
    ...
    if (w < 80) {
      layoutApi && layoutApi.openDetails();
      return;
    }
...
function FileTreePanel({ sessions, workspaces, sessionId }) {
  ...
  React.useEffect(() => {
    const apply2 = () => {
      const ws = resolveCurrentWs(sessionId);
      if (ws) setWorkspace(ws.key, ws.path);
    };
    apply2();
    const un = [];
    try {
      if (sessions && sessions.list) un.push(sessions.list.subscribe(apply2));
      if (workspaces && workspaces.list) un.push(workspaces.list.subscribe(apply2));
    } catch (e) {
      console.error("[fsviewer] subscribe workspace sources:", e);
    }
  ...
  React.useEffect(() => {
    ...
    openPanelWithRoom();
    setTimeout(() => {
      if (currentWs === wsKey && p.panelOpen) openPanelWithRoom();
    }, 80);
  }, []);
  React.useEffect(() => {
    const p = P();
  ...
  ))), kind === "chat" ? /* @__PURE__ */ React.createElement(ChatPanel, { chatId: activeTab.id }) : kind === "browser" ? /* @__PURE__ */ React.createElement(BrowserPane, { tabId: activeTab.id }) : kind === "empty" ? /* @__PURE__ */ React.createElement(EmptyTabsState, null) : /* @__PURE__ */ React.createElement(FilesView, { key: wsKey, root: wsPath, visible }));
}
function FilesView({ root, visible }) {
  ...
  const onTreeResizeStart = (e) => {
    ...
    const startW = treeW;
    const move = (ev) => {
      setTreeW(Math.min(Math.max(startW + (startX - ev.clientX), 120), 320));
    };
    const up = () => {
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "4px 6px 4px 10px", borderBottom: "1px solid " + V.line, flex: "0 0 auto" } }, /* @__PURE__ */ React.createElement("span", { style: { flex: "1 1 auto", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 12, color: V.muted } }, state.root ? baseName(state.root) : "\u2026", state.activePath ? /* @__PURE__ */ React.createElement("span", null, /* @__PURE__ */ React.createElement("span", { style: { color: V.edge } }, " \u203A "), /* @__PURE__ */ React.createElement("span", { style: { color: V.fg } }, baseName(state.activePath))) : null), showSourceBtn ? /* @__PURE__ */ React.createElement(
    ...
    "\u6253\u5F00"
  ) : null), /* @__PURE__ */ React.createElement("div", { style: { flex: "1 1 auto", display: "flex", minHeight: 0 } }, state.activePath ? /* @__PURE__ */ React.createElement(FilePreview, { state }) : /* @__PURE__ */ React.createElement(EmptyState, null), treeOn ? /* @__PURE__ */ React.createElement(TreeColumn, { state, dispatch, width: treeW, onResizeStart: onTreeResizeStart, tabKind: kind }) : null));
}
